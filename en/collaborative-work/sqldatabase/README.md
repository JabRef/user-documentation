...
Shared databases that are still connected when you quit JabRef are reconnected automatically on the next start, just like local libraries (when **Open last edited libraries on startup** is enabled in the preferences). This only works if the password is available, i.e. **Remember password** was ticked and the credential store can be reached. Otherwise the connection fails on startup and JabRef shows an error; connect again via **File → Shared database → Connect to shared database**.
