...
When you open a library that still stores special field values as keywords (for example `prio1` or `rank3`), JabRef offers to move them to separate fields. The dialog lists every conversion of the library to the current format; deselect a conversion to keep the data unchanged. JabRef remembers deselected conversions in the library and does not offer them again. To see the dialog again, remove the `skippedMigrations` comment from the `.bib` file.
