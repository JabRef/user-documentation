...
5. The Hardware recommendation to maximize prompt processing and token generation speed is a device with high _bandwidth and/or one with a neural processing unit (NPU). To date, a modern GPU with lots of VRAM fits the former need best.
...
2. "Expert Settings" has a field called "API base URL" where you need to provide the address of an OpenAI-compatible API server
...
5. After that, you can run ollama serve to start a local web server. This server will accept requests and respond with LLM output. Note: The ollama server may already be running, so do not worry about a cannot bind error. If the server is not yet running, use the following command: `ollama run <MODEL>:<PARAMETERS>`
